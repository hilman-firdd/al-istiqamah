	...
	<div class="box" id="check-out">
		...
		<form action="{{url('admin/checkout/edit')}}" method="POST">
				...
				<div class="row">
					...
					<div class="col-sm-3">
						<div class="form-group">
							<label># INVOICE</label>
							<input class="form-control" name="nomor_invoice" value="{{$transaksi->invoice_id}}"
								readonly="">
						</div>
						<div class="form-group">
					...
					<div class="col-sm-5">
						<div class="form-group">
							...
							<div class="row">
								<div class="col-sm-6">
									<input class="form-control" value="{{$transaksi->jumlah_dewasa}} Dewasa"
										readonly="">
								</div>
								<div class="col-sm-6">
									<input class="form-control" value="{{$transaksi->jumlah_anak}} Anak-anak"
										readonly="">
								</div>
							</div>
						</div>
							...
							<div class="row">
								<div class="col-sm-6">
									<input class="form-control"
										value="{{date('Y-m-d',strtotime($transaksi->tgl_checkin))}}" readonly="">
								</div>
								<div class="col-sm-6">
									<input class="form-control"
										value="{{date('H:i:s',strtotime($transaksi->tgl_checkin))}}" readonly="">
								</div>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="row">
								<div class="col-sm-6">
									<input id="checkout" class="form-control" name="tanggal_checkout"
										...
										value="{{date('Y-m-d',strtotime($transaksi->tgl_checkout))}}" readonly="">
								</div>
								<div class="col-sm-6">
									<input class="form-control" name="waktu_checkout"
										value="{{date('H:i:s',strtotime($transaksi->tgl_checkout))}}" readonly>
								</div>
							</div>
						</div>
					</div>
				...
				<table class="table table-striped">
					...
					<tbody>
						<tr>
							...
							<td class="pull-right">Rp. {{ number_format($transaksi->kamar->typekamar->harga_malam *
								$jumlah_hari,2) }}</td>
			...
			<div class="box-footer">
				<a class="btn btn-warning" href="{{ url('/admin/dashboard')}}">Kembali</a>
			</div>
		</form>
	</div>
</section>
